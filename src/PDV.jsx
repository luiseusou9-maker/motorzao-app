...
const PDV = ({ session }) => {
  ...
  const [novoCarro, setNovoCarro] = useState({ 
    nome: '', preco: '', km: '', cambio: 'Automático', 
  ...
  // --- EXTRAÇÃO INTELIGENTE (FILTRO ANTI-LOGO) ---
  const dissecarLink = async () => {
    if (!novoCarro.link_fonte) return alert("Por favor, insira o link do veículo.");
    setCapturando(true);
    
    try {
      const proxyUrl = `https://api.allorigins.win/get?url=${encodeURIComponent(novoCarro.link_fonte)}`;
      ...
      const titulo = doc.querySelector('h1')?.innerText || "VEÍCULO CATALOGADO";
      const precoMatch = html.match(/R\$\s?([0-9.]+,[0-9]{2}|[0-9.]+)/);
      
      const imagensFiltradas = Array.from(doc.querySelectorAll('img'))
        .map(img => img.src)
        .filter(src => {
          const isUrlValida = src.includes('http') && src.length > 60;
          const isNotLogo = !src.toLowerCase().includes('logo') && 
                           !src.toLowerCase().includes('banner') && 
                           !src.toLowerCase().includes('icon');
          return isUrlValida && isNotLogo;
        });

      // Aqui ele sugere, mas você ainda pode mudar na mão no input antes de salvar
      const nomeLojaSugerido = doc.title.split('-')[0].trim() || "Estacionamento Parceiro";
      ...
      setNovoCarro(prev => ({
        ...
        nome_estacionamento: nomeLojaSugerido,
      ...
      alert("Dados extraídos! Logos descartadas.");

    } catch (e) {
      alert("Erro na extração. Verifique o link.");
    } finally {
      setCapturando(false);
    }
  };
  ...
  const adicionarCarro = async (e) => {
    ...
    await supabase.from('cars').insert([{
      ...
      loja_nome: novoCarro.nome_estacionamento, // Salva o que você escreveu na mão
      zap_origem: novoCarro.zap_loja
    }]);

    // Limpa os campos mas você pode reescrever a loja se for cadastrar outro da mesma
    setNovoCarro({ nome: '', preco: '', km: '', cambio: 'Automático', combustivel: 'Flex', modalidade: 'financiamento', descricao: '', link_fonte: '', zap_loja: '', nome_estacionamento: novoCarro.nome_estacionamento, img1: '', img2: '', img3: '', img4: '', img5: '', img6: '' });
    carregarEstoque();
    setLoading(false);
  };
  ...
  return (
    <div className="min-h-screen bg-[#050505] text-white p-6 font-sans">
      <div className="max-w-7xl mx-auto">
        ...
        {/* BARRA DE CAPTURA - VOLTOU O BOTÃO ORIGINAL */}
        <div className="mb-8 bg-gradient-to-r from-orange-600 to-red-600 p-[1px] rounded-3xl shadow-[0_0_30px_rgba(234,88,12,0.2)]">
          <div className="bg-black p-6 rounded-[1.7rem] flex flex-col md:flex-row gap-4">
            <input 
              placeholder="COLE O LINK DO VEÍCULO (OLX, WEB MOTORS, SITE DE LOJA)" 
            ...
            <button onClick={dissecarLink} className="bg-orange-600 hover:bg-orange-500 text-white font-black px-8 py-3 rounded-xl text-[10px] uppercase italic transition-all">
              {capturando ? 'LIMPANDO LOGOS...' : 'CAPTURAR MÁQUINA'}
            </button>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          <form onSubmit={adicionarCarro} className="bg-[#0a0a0a] p-8 rounded-[2.5rem] border border-white/5 space-y-4 h-fit sticky top-6">
            ...
            {/* Campo da Loja - Escreva na mão aqui */}
            <input type="text" placeholder="Nome da Loja/Estacionamento" value={novoCarro.nome_estacionamento} onChange={e => setNovoCarro({...novoCarro, nome_estacionamento: e.target.value})} className="w-full bg-white/5 p-4 rounded-xl text-xs border border-white/10 outline-none focus:border-orange-600 font-bold" required />
            ...
            <div className="grid grid-cols-2 gap-2">
              ...
              <input type="text" placeholder="KM" value={novoCarro.km} onChange={e => setNovoCarro({...novoCarro, km: e.target.value})} className="bg-white/5 p-4 rounded-xl text-xs border border-white/10 outline-none focus:border-orange-600" />
            </div>
            ...
            <div className="grid grid-cols-2 gap-2">
              ...
              <input type="text" placeholder="WhatsApp da Loja" value={novoCarro.zap_loja} onChange={e => setNovoCarro({...novoCarro, zap_loja: e.target.value})} className="bg-white/5 p-4 rounded-xl text-[10px] border border-white/10 outline-none" />
            </div>
          ...
          {/* LISTA AGRUPADA - TUDO JUNTINHO POR LOJA */}
          <div className="lg:col-span-2 space-y-12">
            {lojasUnicas.map(loja => (
              <div key={loja} className="space-y-4">
                <div className="flex items-center gap-4">
                  <h3 className="text-sm font-black uppercase italic text-orange-600 tracking-tighter">{loja || 'Sem Identificação'}</h3>
                  <div className="h-[1px] flex-1 bg-white/5"></div>
                  <span className="text-[10px] text-gray-600 font-bold uppercase">{carros.filter(c => c.loja_nome === loja).length} VEÍCULOS</span>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {carros.filter(c => c.loja_nome === loja).map(c => (
                    <div key={c.id} className="bg-[#0a0a0a] p-4 rounded-3xl border border-white/5 flex gap-4 items-center group relative overflow-hidden">
                      ...
                      <button 
                        onClick={async () => { if(confirm('Remover?')) { await supabase.from('cars').delete().eq('id', c.id); carregarEstoque(); } }}
